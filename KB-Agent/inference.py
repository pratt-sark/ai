...
class KnowledgeBasedAgent:
  ...
  def choose_location(self, x , y, cave_size):
    # Complete the implementation of the function choose_location(self, x , y, cave_size) 
    # in the inference.py file. The parameters are the current agent location and the cave
    # size and it must return the next safe location, if any. The choice made should be 
    # logical based on inference methods.

    unvisited_locations = self.unvisited()
    safe_moves = self.safe().intersection(unvisited_locations)
    if safe_moves:
      location = safe_moves.pop()
    else: 
      not_unsafe_moves = self.not_unsafe().intersection(unvisited_locations)
      if not_unsafe_moves:
        location = not_unsafe_moves.pop()
      else:
